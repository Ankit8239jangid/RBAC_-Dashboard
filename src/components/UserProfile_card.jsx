...
function UserProfile() {
    ...
    return (
        <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 gap-5 mg:p-6 p-4 w-full  rounded-xl max-w-5xl'>
            {filteredUsers.map((user, index) => (
                <div key={index} className=" transition-transform duration-300 hover:scale-105 hover:shadow-2xl shadow-md shadow-gray-400 relative flex flex-col md:p-2 p-2 border border-gray-300 rounded-xl w-full mx-auto">
                    ...
                    <div className="flex items-center justify-between mb-2">
                        ...
                        <span className={`px-2  font-semiboldrounded-full ${user.status ? "text-green-600" : "text-red-500"}`}>
                        ...
                        <span className="py-3 px-6 ">
                            ...
                            {dropdownState.action === index && (
                                <div className="absolute  right-20 top-36  w-36 bg-gray-200 rounded-2xl">
                                    <button
                                        className=" transition-transform duration-300 hover:scale-105 hover:shadow-2xl rounded-2xl flex items-center gap-2 w-full text-left px-4 py-2 text-gray-900 hover:bg-gray-300"
                                        onClick={() => handleActionWithDropdownReset("Edit", user)}
                                    >
                                        <MdEdit /> Edit
                                    </button>
                                    <button
                                        className=" transition-transform duration-300 hover:scale-105 hover:shadow-2xl flex items-center rounded-2xl gap-2 w-full text-left px-4 py-2 text-red-600  hover:bg-gray-300"
